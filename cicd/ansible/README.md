...
## 5. `cleanup-selective.sh` — Azure 리소스 선택적 삭제

**역할**: Azure 리소스를 **의존성 역순**으로 안전하게 삭제

### 이번에 추가된 부분

| 단계 | 추가 내용 | 이유 |
|------|-----------|------|
| [11] 테스트 VM 삭제 | VM → NIC → Public IP → OS Disk 순서로 삭제 | VM이 snet-services 서브넷 점유 → VNet 삭제 블로킹 |
| [12] VNet 사전 정리 1 | Bastion Host(`vnet-bookflow-bastion`) 삭제 | Bastion이 VNet 참조 → VNet 삭제 블로킹 |
| [12] VNet 사전 정리 2 | snet-services 서브넷 고아 NIC 일괄 삭제 | VM 없이 NIC만 남은 경우도 처리 |
...
→ [11] 테스트 VM + NIC + Disk
→ [12] Bastion + 고아 NIC → VNet
→ [13] NSG → [14] 관리 ID → [15] ARM 배포 이력
```

---

## 6. `vpn-connect.sh` — AWS ↔ Azure VPN 자동 연결

**역할**: AWS ↔ Azure VPN을 **완전 자동**으로 구성 (수동 입력 최소화)

### 기존 vs 변경

| 항목 | 기존 | 변경 후 |
|------|------|---------|
| AWS ASN | 수동 입력 | `tgw.yaml` 파싱 → 자동 (`64512`) |
| BGP Peer IP | 수동 입력 | `vpn-site-to-site.yaml` Tunnel CIDR 파싱 → 자동 (`169.254.21.5`) |
| VPC CIDR | 수동 입력 | `vpc-*.yaml` 4개 파싱 → 자동 |
| TGW 공인 IP | 수동 입력 | AWS CLI CloudFormation 스택 조회 → 자동 |
| PSK | 수동 입력 | AWS VPN Connection XML에서 자동 추출 |
| BGP 설정 | 없음 | `--asn`, `--bgp-peering-address` 포함 |

### 자동 파싱 흐름

```
CloudFormation YAML 파일 읽기
    → AWS_ASN, BGP Peer IP, VPC CIDRs 추출
AWS CLI 호출
    → TGW Outside IP, PSK 조회
Azure CLI 실행
    → Local Network Gateway 생성 (BGP 포함)
    → VPN Connection 생성 (--enable-bgp)
    → 연결 상태 자동 검증
```
...
az network vpn-connection show \
  ...
  --name conn-bookflow-aws-active \
  --query "{status:connectionStatus, bgp:enableBgp}" \
  --output table
