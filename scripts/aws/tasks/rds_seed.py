...
for t in {load_order}; do
  ...
  # column-aware \copy: csv header → DB column list (DB 추가 컬럼은 default · 매일 schema 변동 시 generate.py csv 가 source of truth)
  HEADER=$(head -1 "$csv")
  $PSQL -At -c "\\copy $t ($HEADER) FROM '$csv' WITH (FORMAT csv, HEADER true);" > /dev/null
