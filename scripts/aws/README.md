...
| 스크립트 | 대상 | 비고 |
|---|---|---|
| `1-daily/base-down.ps1` | Tier 10-60 모든 stack 일괄 (Tier 0 제외) | 매일 18:00 |
| `0-initial/phase0-foundation-down.ps1` | Tier 00 영구 자원 (KMS · Secrets · S3 · ECR 등) | ⚠️ 강력 confirmation · 학기 종료 시 |
| `0-initial/wipe-all.ps1` | base-down + phase0-foundation-down | ⚠️ 가장 강력 · 프로젝트 완전 종료 |
| `2-tasks/task-data-down.ps1` | RDS + Redis + Kinesis | 데이터 영구 삭제 |
| `2-tasks/task-msa-pods-down.ps1` | EKS + Nodegroup + Addons + IRSA + endpoints + peering | |
| `2-tasks/task-etl-streaming-down.ps1` | ECS sims + endpoints + peering | |
| `2-tasks/task-publisher-down.ps1` | Publisher ASG + inventory-api + ALB + WAF + peering | |
| `2-tasks/task-auth-pod-down.ps1` | NAT + VPN (Azure 부분) + TGW (공유 자원 체크) | |
| `2-tasks/task-forecast-down.ps1` | VPN (GCP 부분) + TGW (공유 자원 체크) | |
| `2-tasks/task-lambdas-down.ps1` | SAM stack 단일 destroy (7 Lambdas + 트리거) | |
| `2-tasks/task-glue-down.ps1` | Glue Database + 6 Jobs + Step Functions | |
| `2-tasks/task-client-vpn-down.ps1` | Client VPN Endpoint | |
| `2-tasks/task-rds-seed-down.ps1` | peering-ansible-data | |
| `2-tasks/task-full-stack-down.ps1` | 모든 task 자원 (base 제외) · wrapper | 시연 후 정리 |
| `2-tasks/task-scenario-ha-revert.ps1` | HA 모드 → 구축 모드 (Single-AZ 복귀) | scenario 후 |
