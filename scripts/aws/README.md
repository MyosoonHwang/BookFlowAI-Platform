...
### Tier 50 Network Traffic (task-publisher · 추후 작성)

```
31. nat-gateway            ← task-auth-pod (Auth Pod outbound)
32. alb-external           ← task-publisher (재고조회 + Publisher Blue/Green)
33. waf                    ← task-publisher (External ALB attach)
```

### Tier 60 Network Cross-Cloud (task-auth-pod · task-forecast)

```
34. tgw                    ← Phase 3 통합 시나리오
35. vpn-site-to-site       ← task-auth-pod (Azure) · task-forecast (GCP)
36. client-vpn             ← Phase 2 (담당자 OIDC)
```
